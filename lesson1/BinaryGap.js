...
representation 10100 and contains one binary gap of length 1. The number 15 has binary 
representation 1111 and has no binary gaps. The number 32 has binary representation 
100000 and has no binary gaps.
Write a function:
function solution(N); that, given a positive integer N, returns the length of its longest 
...
function should return 0, because N has binary representation '100000' and thus no binary 
gaps. 
Write an efficient algorithm for the following assumptions: N is an integer within 
the range [1..2,147,483,647].
*/
function solution(N) {
    // Implement your solution here
    let binaryNum = (N >>> 0).toString(2); // Convert Decimal Number to Binary number
    let maxGap = 0;

    for (let i = 0; i < binaryNum.length; i++) {
        for (let j = 0; j < binaryNum.length; j++) {
            if (binaryNum[i] == 1 && binaryNum[j] == 1) {
                if (maxGap < j - i - 1) {
                    maxGap = j - i - 1
                }
                i = j
...
// console.log(binaryGap(1041)) // return 5